		...
		while True:
			...
			if Correct:
				if self.active_notes:
					self.realtime_note_label.config(fg='white', text=f"input: {sorted(self.active_notes)}")
				else:
					self.realtime_note_label.config(fg='white', text=f"input: ")
			else:
				if self.active_notes:
					self.realtime_note_label.config(fg='red', text=f"input: {sorted(self.active_notes)}")
				else:
					self.realtime_note_label.config(fg='red', text=f"input: ")
			...
			if Missed:
				if self.NOTE_ON_golden:
					self.golden_note_label.config(fg='red', text=f"golden: {sorted(self.NOTE_ON_golden)}")
				else:
					self.golden_note_label.config(fg='red', text=f"golden: ")
			else:
				if self.NOTE_ON_golden:
					self.golden_note_label.config(fg='white', text=f"golden: {sorted(self.NOTE_ON_golden)}")
				else:
					self.golden_note_label.config(fg='white', text=f"golden: ")

			# self.scroll_and_draw_original(elapsed_beat)
